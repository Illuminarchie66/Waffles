    ...
    private void testDataCheckerOnValidRestaurant() {
        try {
            ...
            Restaurant[] restaurants = restaurantStore.loadRestaurantDataToArray(
                    loadData("/test-restaurant/restaurant-10-damascus.csv"));

            System.out.println(DataChecker.isValid(restaurants[0]));
    ...
    private void testStringFormatterCorrect() {
        try {
            String input = "ABCDEFGHIJKLMNOPQRSTUVWXYZ\n" +
                           ...
                           "1234567890\n" +
                           "!£$%^&*-_+=()'{}:;~@#<>?,./|`¬\n" +
                           "ÀÈÌÒÙ ÁÉÍÓÚÝ ÂÊÎÔÛ ÃÑÕ ÄËÏÖÜŸ\n" +
                           "àèìòù áéíóúý âêîôû ãñõ äëïöüÿ\n" +
                           "Æ ß ŋ";
            String output = StringFormatter.convertAccents(input);
            boolean result = StringFormatter.convertAccentsFaster(input).equals(output);
    ...
    private void testStringFormatterIsFaster() {
        try {
            ...
            for (int n = 0; n < warmUpRuns; n++) {
                result = StringFormatter.convertAccents(input).equals(output);
                if (!result) {
                    break;
                }
            ...
            for (int n = 0; n < warmUpRuns; n++) {
                result = StringFormatter.convertAccentsFaster(input).equals(output);
                if (!result) {
                    break;
                }
